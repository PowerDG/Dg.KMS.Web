...
- 前端代码生成的文件文件路径请在Cube.Application类库中生成的对应实体代码文件中的Client中。
...
如果你选择了**生成权限功能**，请打开Cube.Application类库中的CubeApplicationModule.cs类文件。
然后复制以下代码到 的PreInitialize 方法中:

```csharp
Configuration.Authorization.Providers.Add<EvernoteTagAuthorizationProvider>();
...
请打开Cube.Application类库中CubeApplicationModule.cs中的 PreInitialize 方法中:
...
打开EntityFrameworkCore类库在 **CubeDbContext**类文件中添加以下代码段：
...
{
   modelBuilder.ApplyConfiguration(new EvernoteTagCfg());
 }
...
```
Add-Migration AddNewEvernoteTagEntity_Migration
```
...
生成的多语言内容
在Cube.Core类库中对应实体下的`douyuyan.md`文件中。
